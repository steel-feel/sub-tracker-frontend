<script>
    ...
    let shipId = "", status="", origin="", destination="";
...
<div class="text-base font-bold grid grid-cols-2 gap-4">
    ...
    <div class="my-4" >Status  <span class:bg-yellow-200={status === 'InTransit'} class:bg-green-200={status === 'Delivered'} class="px-4 py-2 m-3 text-base rounded-full    ">
        {status}
    </span> </div>
    <div></div>
    ...
    >
        <ul class="flex my-3 flex-col divide divide-y">
            {#each routes as route, i}
                <li class:bg-indigo-100={ownerIndex - 1 == i} class="flex  flex-row">
                    <div
                        class="select-none flex flex-1 items-center p-4"
                    >
